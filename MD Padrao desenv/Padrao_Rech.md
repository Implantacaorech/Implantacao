# Padrões de Desenvolvimento — Rech Informática Ltda

**Revisão:** `2.0.0` · **Data:** 21/07/2026 · **Status:** Vigente · **Responsável:** Time DevTools

> Documento normativo destinado à leitura por agentes de IA (Claude Code, Copilot, Cursor e equivalentes)
> que atuem sobre diretórios de projeto de software da Rech Informática Ltda.
>
> Fonte primária: [Procedimentos para desenvolvimento de aplicações na Rech](http://intranet/blog/48347/procedimentos-para-desenvolvimento-de-aplicacoes-na-rech)
> (Blog da Intranet, por Sandro, 04/06/2026), complementada pelas convenções observadas no
> GitLab interno em `https://gitlab.rech.com.br/gitlab/rech/`.
...
|---|---|---|
| **Arquivo presente no projeto** | Cópia do `PADRAO-RECH.md` na raiz ou em `docs/` | Simples, porém envelhece em silêncio |
| **Carregado por URL** | O agente busca a versão canônica sob demanda | Sempre atualizado — **forma preferida** |
| **Referenciado** | `CLAUDE.md`, `AGENTS.md` ou regras do agente apontam para a URL, e o `README.md` registra a vinculação | Combina rastreabilidade e atualização automática |

**Prefira referenciar a copiar.** Uma cópia local congela a versão do dia em que foi feita e passa a
divergir do padrão sem que ninguém perceba. Havendo cópia local em conflito com a versão canônica,
**vale a canônica** — e o agente deve avisar o humano que a cópia está desatualizada.

**Como verificar a atualidade.** Compare o campo **Revisão** do cabeçalho da cópia local com o da
versão canônica (§13). Divergindo, use a canônica e informe o humano citando as duas revisões. Se a
canônica estiver inacessível, prossiga com a cópia local e registre no relatório que a atualidade
...
Para vincular um projeto ao padrão sem copiá-lo, inclua no `CLAUDE.md` / `AGENTS.md` do projeto:
...
1. **Auditar** o projeto atual contra o checklist da seção 2 e as regras das seções 3 a 10.
...
6. **Vinculação a este padrão** — uma linha declarando que o projeto o segue, com link para a URL
   canônica (§1.1). É o que faz um agente de IA, em qualquer sessão futura, descobrir sozinho que o
   projeto está sujeito a estas regras. Sem essa linha, cada nova conversa começa do zero e a
   conformidade depende de alguém lembrar de mencioná-la.
...
Ao concluir a auditoria, o agente deve produzir um relatório neste formato:

```markdown
## Conformidade com os padrões Rech — <nome-do-projeto>
...
- <Lista resumida dos itens do checklist aprovados>

### Plano de adequação — aguardando confirmação
1. **[agente]** <ação que o agente executa sozinho assim que autorizado>rpr
...
Critério de numeração, alinhado ao `bump-version` do `ri-ci-cd` (§7.2):
...
revisão no cabeçalho e registrar a linha correspondente nesta tabela.
