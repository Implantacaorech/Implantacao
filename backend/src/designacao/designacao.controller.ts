...
import { PERFIS_AGENDAMENTO, PERFIS_DESIGNA_CONSULTORES } from '../common/constants/perfis';
...
export class DesignacaoController {
  ...
  @ApiOperation({ summary: 'Etapa 5 (Administrativo): dados para definir o(s) GCI(s)' })
  ...
  @ApiOperation({ summary: 'Etapa 5 (Administrativo): define o(s) GCI(s) — sem notificar ainda' })
  async definirGci(
    @Param('id', ParseIntPipe) id: number,
    @Body() dto: DefinirGciDto,
    @CurrentUser() user: AuthUser,
  ) {
    return new ApiEnvelope(await this.service.definirGci(id, dto.gcis, user.nome));
  }

  @Get('agendar')
  @Roles(...PERFIS_AGENDAMENTO)
  @ApiOperation({ summary: 'Etapa 2 (Administrativo): dados para agendar o Levantamento' })
  ...
  @HttpCode(HttpStatus.OK)
  @ApiOperation({
    summary: 'Etapa 2 (Administrativo): confirma a data do Levantamento, notifica o(s) GCI(s) e auto-avança a etapa',
  })
  async agendar(
    @Param('id', ParseIntPipe) id: number,
    ...
    return new ApiEnvelope(await this.service.agendar(id, dto.dataLevantamento, user.nome));
  }

  @Get('consultores')
  @Roles(...PERFIS_DESIGNA_CONSULTORES)
  @ApiOperation({ summary: 'Etapa 6 (GCI): dados para designar os consultores por módulo' })
  ...
  @HttpCode(HttpStatus.OK)
  @ApiOperation({
    summary: 'Etapa 6 (GCI): designa os consultores, notifica cada um e auto-avança a etapa',
  })
  async designarConsultores(
    ...
    return new ApiEnvelope(await this.service.designarConsultores(id, dto.designacoes, user.nome));
  }
}
