...
describe('LevantamentoRespostaService', () => {
  ...
  const repo = { find: jest.fn(), save: jest.fn(), create: jest.fn((dto) => dto), count: jest.fn() };
  ...
  describe('importarDeParagrafos', () => {
    it('preenche a resposta com o texto depois do tópico, na mesma linha', async () => {
      ...
      const n = await service.importarDeParagrafos(1, ['Razão Social: Cliente Teste Ltda']);
      expect(n).toBe(1);
    ...
    it('ignora placeholder de modelo em branco ("<...>")', async () => {
      ...
      repo.find.mockResolvedValue(linhas);
      const n = await service.importarDeParagrafos(1, ['Fornecedor Atual: <preencher>']);
      expect(n).toBe(0);
      expect(linhas[0].resposta).toBe('');
    });

    it('não sobrescreve com string vazia quando nenhum parágrafo menciona o tópico', async () => {
      const linhas = [{ id: 1, topico: 'Localização', resposta: 'já preenchida antes' }];
      repo.find.mockResolvedValue(linhas);
      const n = await service.importarDeParagrafos(1, ['Nada a ver com o tópico']);
      expect(n).toBe(0);
    ...
    it('usa o primeiro parágrafo que casar, na ordem recebida', async () => {
      ...
      repo.find.mockResolvedValue(linhas);
      await service.importarDeParagrafos(1, ['linha irrelevante', 'Ramo: Comércio', 'Ramo: Serviços']);
