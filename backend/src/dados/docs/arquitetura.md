...
Consumidor (Painel · outro sistema · agente de IA · BI)
  ...
  DadosController ─────────── AcessoDadosGuard
        │                       ├─ X-API-Key → ClienteApiService (escopo)
...
| `dados-admin.controller.ts` | `/api/dados/v1/admin` — clientes de máquina, métricas, cache (ADM) |
...
| `catalogo/catalogo.ts` | **O catálogo** — as 19 consultas declaradas |
...
| `decorators/chamador.decorator.ts` | Identidade e escopos do chamador |

## A fronteira, em três fases — todas concluídas em 2026-08-25
...
volta do painel de Permissões. Máquina entra por `X-API-Key` e é gateada por **escopo** —
não tem tela nem perfil, e um JWT de 15 minutos não serve para integração.
...
E só. **Nenhum módulo de negócio** — a seta aponta sempre para cá.
