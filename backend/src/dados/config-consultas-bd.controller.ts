...
/** Consultas BD (SQLs nomeadas contra a conexão externa) — exclusivo do Administrador, e
 * desde 2026-08-26 exclusivo do **Portal API**: o usuário tirou a tela do Portal Implantação
 * ("o uso será único e exclusivo no Portal API"). Por isso o controller mudou de
 * `disponibilidade/` para cá — é o `DadosModule` que a instância interna monta; ficando lá,
 * a tela existiria no portal errado e faltaria no certo.
 *
 * Contexto original: exclusivo do Administrador,
