...
describe('DisponibilidadeService', () => {
  ...
  describe('config', () => {
    ...
    it('salva e recarrega, preservando a senha quando reenviada em branco', () => {
      service.salvarConfig({ host: 'db.exemplo', banco: 'SICLA', senha: 'segredo', ativo: true });
      service.salvarConfig({ host: 'db.exemplo', banco: 'SICLA', senha: '', ativo: true });
      const cfg = service.carregarConfig();
    ...
    it('configurado exige ativo + select + conexão montável', () => {
      service.salvarConfig({ host: 'db.exemplo', banco: 'S', select: 'SELECT 1', ativo: false });
      ...
      service.salvarConfig({ host: 'db.exemplo', banco: 'S', select: 'SELECT 1', ativo: true });
  ...
  describe('filtraPorTecnico', () => {
    it('detecta :tecnicos como token isolado, não como prefixo de outro nome', () => {
      expect(service.filtraPorTecnico({ ...service.carregarConfig(), select: 'WHERE cod IN :tecnicos' })).toBe(true);
      expect(
        service.filtraPorTecnico({ ...service.carregarConfig(), select: 'WHERE cod IN :tecnicos_outros' }),
      ).toBe(false);
      expect(service.filtraPorTecnico({ ...service.carregarConfig(), select: 'SELECT 1' })).toBe(false);
    });
  });
  ...
  describe('consultar / executarSql — binds e expansão de :tecnicos', () => {
    it('consultar expande :tecnicos em binds nomeados e normaliza a linha (case-insensitive)', async () => {
      const execute = jest.fn().mockResolvedValue({
        rows: [{ TECNICO: 'Ana', DATA: '2026-08-10T00:00:00.000Z', TURNO: 'MANHA' }],
      });
      mockedOracledb.getConnection.mockResolvedValue(conexaoFake(execute));
      service.salvarConfig({
        host: 'db.exemplo',
        banco: 'S',
        select: 'SELECT t AS tecnico, d AS data, tu AS turno FROM x WHERE cod IN :tecnicos',
        ativo: true,
      });

      const linhas = await service.consultar('2026-08-01', '2026-08-31', ['123', '456']);
      expect(linhas).toEqual([{ tecnico: 'Ana', data: '2026-08-10', turno: 'manha' }]);
    ...
    it('lista de técnicos vazia vira (NULL) — nunca casa, sem binds extras', async () => {
      ...
      expect(bindsChamados).toEqual({ data_ini: '2026-08-01', data_fim: '2026-08-31' });
    });
    ...
    it('executarSql ignora comentários iniciais ao validar SELECT/WITH', async () => {
      ...
      mockedOracledb.getConnection.mockResolvedValue(conexaoFake(execute));
      service.salvarConfig({ host: 'db.exemplo', banco: 'S', select: 'SELECT 1', ativo: true });
      const r = await service.executarSql('-- comentário\n/* bloco */\nSELECT 1 FROM dual');
    ...
    it('executarSql traduz erro DPY-3015 (verificador de senha antigo) numa mensagem amigável', async () => {
      mockedOracledb.getConnection.mockRejectedValue(new Error('DPY-3015: password verifier type is not supported'));
      service.salvarConfig({ host: 'db.exemplo', banco: 'S', select: 'SELECT 1', ativo: true });
  ...
  describe('ocupacaoPorSlot — tradução código/nome e re-indexação por alias', () => {
    it('aceita código OU nome no cadastro e indexa a ocupação pelos dois', async () => {
      const executeTecnicos = jest
        .fn()
        .mockResolvedValue({ rows: [{ CODIGO: '007', TECNICO: 'Ana Consultora' }] });
      ...
      const ocup = await service.ocupacaoPorSlot('2026-08-01', '2026-08-31', ['007']);
  ...
  describe('ocupacaoPorSlotCache', () => {
    it('não repete a consulta dentro do TTL, mesmo com técnicos em ordem diferente', async () => {
      ...
      // consulta nova.
      await service.ocupacaoPorSlotCache('2026-08-01', '2026-08-31', ['A', 'B']);
      expect(mockedOracledb.getConnection).toHaveBeenCalledTimes(2);
      await service.ocupacaoPorSlotCache('2026-08-01', '2026-08-31', ['B', 'A']);
      expect(mockedOracledb.getConnection).toHaveBeenCalledTimes(2);
    });
  });
  ...
  describe('testar', () => {
    ...
    it('erro de conexão devolve mensagem amigável, não a exceção crua', async () => {
      mockedOracledb.getConnection.mockRejectedValue(new Error('ORA-12154: TNS could not resolve'));
      service.salvarConfig({ host: 'db.exemplo', banco: 'S', select: 'SELECT 1', ativo: true });
      const r = await service.testar();
      expect(r.ok).toBe(false);
