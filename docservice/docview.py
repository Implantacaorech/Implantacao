...
Cópia de webapp/docview.py (não importada — mesmo motivo do gerador/gl_*.py: não puxar
a dependência do Flask/SQLAlchemy do schema antigo, ver
docs/migracao/02-decisao-arquitetura.md). Único ajuste: cache local ao docservice em vez
de `_common.DATA_WRITE` (que só existe no app Flask)."""
import os
import html
import threading

_PDF_LOCK = threading.Lock()   # serializa as conversões (Word COM não gosta de concorrência)
_CACHE_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), "_cache_preview")
...
def to_pdf(path):
    ...
    pdf = os.path.join(_CACHE_DIR, key + ".pdf")
